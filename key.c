...
/*   Updated: 2023/09/08 17:21:09 by hlasota          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "fdf.h"
...
{
	if (keycode == 119)
		if (ft_absol(all->map.start_y) + 10 < ft_absol(all->map.start_x))
			all->map.start_y = (all->map.start_y - 10);
	if (keycode == 115)
		if (ft_absol(all->map.start_y) - 10 < ft_absol(all->map.start_x))
			all->map.start_y = (all->map.start_y + 10);
	if (keycode == 97)
		if (ft_absol(all->map.start_x) - 10 > ft_absol(all->map.start_y))
...
int	key_hook(int keycode, t_all *all)
{
	...
	if (keycode == 65362 && all->map.decal_x > 0 && all->map.decal_y > 0)
	{
		all->map.decal_x -= 1;
		all->map.decal_y -= 1;
	}
	...
	if (keycode == 65361 && all->map.angle > 0.044)
		all->map.angle -= 0.02;
	if (keycode == 65363 && all->map.angle < 3.08)
		all->map.angle += 0.02;
	if (keycode == 65307)
		ft_close(*all);
	dessiner(*all);
